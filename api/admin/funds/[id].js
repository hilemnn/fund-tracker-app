  ...
  try {
    switch (req.method) {
      ...
      case 'PUT':
        ...
        if (updateData.totalAmount) updateData.totalAmount = updateData.totalAmount.toString();
        if (updateData.payableAmount) updateData.payableAmount = updateData.payableAmount.toString();
    ...
    console.error('Admin Fund API Error:', error);
    res.status(500).json({ 
      success: false,
